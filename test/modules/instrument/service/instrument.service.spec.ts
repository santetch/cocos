...
describe('Instrument Service', () => {
  ...
  describe('when searching instruments', () => {
    it('should find instruments by query', async () => {
      ...
      jest
        .spyOn(instrumentsService, 'search')
        .mockResolvedValue([{ id: 1, ticker: 'AAPL', name: 'Apple Inc.', type: 'BONO' }]);

      const results = await instrumentsService.search({ query, page, pageSize });

      expect(results).toEqual([{ id: 1, ticker: 'AAPL', name: 'Apple Inc.', type: 'BONO' }]);
    });
  });
    ...
    return await Test.createTestingModule({
      providers: [
        InstrumentsService,
        {
          ...
          useValue: {
            find: jest.fn().mockResolvedValue([{ id: 1, ticker: 'AAPL', name: 'Apple Inc.', type: 'BONO' }]),
            findByIds: jest.fn().mockResolvedValue([{ id: 1, ticker: 'AAPL', name: 'Apple Inc.', type: 'BONO' }]),
            getPricesFor: jest.fn().mockResolvedValue([{ instrumentId: 1, close: '150.00', previousClose: '148.00' }])
          }
        }
      ]
    }).compile();
